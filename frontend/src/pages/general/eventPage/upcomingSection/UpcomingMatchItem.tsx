    ...
    return (
        ...
        >
            ...
            <span className="flex-shrink-0 w-8 h-8 rounded-xl bg-[var(--bg-main)] border border-[var(--border-color)]
                             flex items-center justify-center text-[var(--accent-purple)] dark:text-[#e879f9]">
                <CalendarClock className="w-4 h-4" />
            </span>

            <div className="flex-1 min-w-0">
                <p className="font-black text-[var(--text-primary)] text-sm truncate group-hover:text-[var(--accent-purple)] transition-colors">
                    {match.title}
                </p>
                <div className="flex items-center gap-2 mt-0.5 text-xs font-medium text-[var(--text-primary)] opacity-40">
                    <span>Starts {fmt(match.startTime)}</span>
                </div>
            </div>

            <span className="flex-shrink-0 text-[10px] font-black tracking-widest uppercase
                             text-[var(--accent-purple)] dark:text-[#e879f9] bg-[var(--accent-purple)]/10 border border-[var(--accent-purple)]/20
