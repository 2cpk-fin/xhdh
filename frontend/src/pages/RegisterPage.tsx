    ...
    <div className={`min-h-screen flex items-center relative overflow-hidden transition-colors duration-300 ${bgMain}`}>
      ...
      <div className={`w-full flex justify-end px-12 lg:px-64 z-10 transition-all duration-700 ease-in-out 
        ...
        <div className={`max-w-md w-full rounded-[2.5rem] border p-10 relative transition-all duration-300 ${cardBg}`}>
          <button
            onClick={toggleTheme}
            className={`absolute top-8 right-8 p-3 rounded-xl border transition-all active:scale-95 ${isDark ? 'bg-white/5 border-zinc-800 text-yellow-500' : 'bg-zinc-50 border-zinc-200 text-green-600'
              }`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center justify-center py-4 px-6 rounded-2xl font-bold text-sm transition-all active:scale-[0.98] border mb-6 ${isDark ? 'bg-white/5 border-zinc-800 hover:bg-white/10 text-white' : 'bg-white border-zinc-200 hover:bg-zinc-50 text-zinc-900 shadow-sm'
              }`}
          >
            <GoogleIcon />
            Sign up with Google
