...
const Sidebar = () => {
  ...
  return (
      <aside className={`w-64 h-screen sticky top-0 p-6 flex flex-col border-r transition-all duration-300 ${sidebarBg}`}>
        <div className="flex items-center gap-3 px-3 py-3 bg-purple-500/10 rounded-2xl border border-purple-500/20 mb-8">
          ...
          {links.map((item) => {
            ...
            return (
                <Link
                    key={item.name}
                    to={item.path}
                    className={`group flex items-center gap-4 py-3 px-4 rounded-xl ${
                        isActive
                            ? 'text-white bg-purple-500/20'
                            : sidebarText + ' hover:text-purple-500 hover:bg-purple-500/10'
                    } transition-all duration-200`}
                >
                  <Icon className={`w-5 h-5 ${isActive ? 'text-[var(--highlight)]' : 'group-hover:text-[var(--highlight)]'} transition-colors`} />
        ...
        <div className="space-y-1 border-t border-zinc-800/50 pt-6 mt-6">
          <Link to="/profile" className={`group flex items-center gap-4 py-3 px-4 rounded-xl ${sidebarText} hover:text-purple-500 hover:bg-purple-500/5 transition-all`}>
            <UserCircle className="w-5 h-5 group-hover:scale-110 transition-transform" />
            <span className="font-medium">User</span>
          </Link>
          <Link to="/settings" className={`group flex items-center gap-4 py-3 px-4 rounded-xl ${sidebarText} hover:text-purple-500 hover:bg-purple-500/5 transition-all`}>
