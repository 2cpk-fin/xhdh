...
public class ScheduleMatchLeaderboardServiceImpl implements ScheduleMatchLeaderboardService {

    private final UniversityService universityService;
    private final LeaderboardRedisPort leaderboardRedisPort;
    ...
    public List<ScheduleParticipantResponse> showLeaderboard(String publicMatchId) {
        ...
        Set<ZSetOperations.TypedTuple<Object>> participants;
        participants = leaderboardRedisPort.getParticipants(publicMatchId);
        ...
        for (ZSetOperations.TypedTuple<Object> participant : participants) {
            if (participant != null && participant.getValue() != null) {
                ...
                if (university != null) {
                    ...
                    ScheduleParticipantResponse participantResponse = ScheduleParticipantResponse.builder()
 ...
 * In this scenario, we don't want to call the method show leaderboard thousands
 * of time, this will waste the Upstash resource
 * Using @Cacheable for leaderboard and @CacheEvict for vote, we'll prevent
 * hitting the method too much, and fetch directly from the Redis
 */
/*
 * Case 2: If there's more writers than readers
 * In this case, we'll remove the @Cacheable and @CacheEvict
 * Because many writers are writing at the same time, the network will
 * constantly call the method
