...
public class ScheduleMatchController {
    ...
    public ResponseEntity<List<ParticipantResponse>> getScheduledMatchParticipant(@PathVariable long id) {
        return new ResponseEntity<>(scheduleMatchService.getAllParticipants(id), HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<ScheduleMatchResponse> createScheduledMatch(@Valid @RequestBody ScheduleMatchRequest scheduleMatchRequest) {
        return new ResponseEntity<>(scheduleMatchService.createMatch(scheduleMatchRequest), HttpStatus.CREATED);
    }

    @PutMapping(path = "/{id}")
    public ResponseEntity<ScheduleMatchResponse> updateScheduledMatch(@PathVariable long id, @Valid @RequestBody ScheduleMatchRequest scheduleMatchRequest) {
